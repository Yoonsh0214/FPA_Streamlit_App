...
### 1. Success/Fail 태그
- **Success**: 액션 코드를 2번 반복 (`ss`, `cc`, `aa` 등)
- **Fail**: 액션 코드를 1번만 (`s`, `c`, `a` 등)
- **항상 Success**: `dd`, `ddd` (유효슈팅, 골), `z` (키패스), `gp` (볼획득), `w` (클리어), `qw` (블락), `v` (캐칭), `vv` (펀칭), `sv` (세이브)
- **항상 Fail**: `d` (슈팅), `db` (막힌 슈팅)
- **태그 제외**: `t` (터치), `m` (미스), `q` (인터셉트), `qq` (볼소유), `o` (오프사이드) 등은 자동 성공/실패 태깅에서 제외됩니다.
- **전달 액션 필수값**: `s`, `c`, `z`, `tr` 계열 액션은 반드시 받는 선수(Receiver) 번호가 포함되어야 데이터가 입력됩니다. (예: `10ss8`, `10tr8`)
